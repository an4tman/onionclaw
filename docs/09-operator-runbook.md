...
How you interact with the running SOC agent, and the routine operational tasks. All interaction
happens in your Discord SOC channel (`SOC_DISCORD_CHANNEL`); the `soc` agent handles the commands.
...
> network is clean. See [10-security-model](10-security-model.md).
...
**Which TI providers are enabled** — ask the agent to call `ti_provider_status` (no secrets), or
check which keys are set in the gateway's `ti.env`.

**Rotate SO / TI credentials** — edit the gateway's `so.env` / `ti.env` and recreate the
`mcp-so-gateway` container (see [03-mcp-deployment](03-mcp-deployment.md)). Tuning tokens reset on
restart.
...
|---|---|
| "Approved a tuning, nothing applied" | The SOC channel must be bound to the **cloud `soc` agent**, not a local model (which loops and never calls `apply_tuning`). Confirm the Discord bind ([04-openclaw-setup](04-openclaw-setup.md)). |
...
| Cycle missed its run | OpenClaw cron can drop a job across a version update. Confirm the cron job exists, is scheduled on `SOC_CYCLE_CRON`, and is pinned to `agent_id = soc`. |
| `get_detection` empty for a Sigma UUID | Stale gateway image — rebuild it. `get_detection` falls back from ES `_id` to `publicId`; a UUID *should* resolve. Don't conclude "the gateway can't handle Sigma." |
...
- Suppress **behavior**, never a host. Every `approve` you grant should be a narrow, behavior-specific
  proposal — if a proposal looks host-wholesale, reject it and ask for a narrower scope.
- Update your `environment.md` as the network changes (new hosts, new benign baselines). Stale
  grounding is the main source of misclassification.
