...
| Grounding writes are gated the same way | The analyst's `environment.md` is written only through `propose_grounding` → your approval → `apply_grounding`. Append-only under known section headings (entries containing headings are rejected), audited with prior file contents, revertible. The cycle can't even propose: it surfaces `GROUNDING GAP` lines for you to answer. |
| KB writes are gated the same way, edits doubly | With `KB_WRITE_ROOT` set, wiki changes ride the propose → approve → apply gate. Appends only add under existing headings; edits replace one exact-unique match, return `double_gated`, and the workflow demands a second confirmation. Apply re-validates against the current page (no clobbering a page that changed), everything is audited with prior content, and reverts are targeted. |
